...
<section id="sidebar">
                    ...
                    <header>
                        ...
                        @if(!is_null($postsMostCommented->date))
                            <time class="published"
                                  datetime="{{$postsMostCommented->date}}">{{$postsMostCommented->date}}</time>
                        @else
                            <time class="published"
                                  datetime="{{$postsMostCommented->created_at->format('d/m/Y')}}">{{$postsMostCommented->created_at->format('d/m/Y')}}</time>
                        @endif
                        @if(!empty($post))
                            <a href="#" class="author"><img src="/assets/images/users/{{$post->user->url_thumbnail}}"
                                                            alt=""/></a>
                        @endif
                    </header>
                    @if(!is_null($postsMostCommented->url_thumbnail))
                        <a href="/article/{{$postsMostCommented->id}}" class="image"><img
                                    src="/assets/images/posts/{{$postsMostCommented->url_thumbnail}}" alt=""/></a>
                    @endif
                </article>
            @empty
            @endforelse
    ...
    <section class="blurb">
        ...
        <ul class="actions">
            <li><a href="{{url('/mentions')}}" class="button">Lire la suite...</a></li>
        </ul>
    </section>
